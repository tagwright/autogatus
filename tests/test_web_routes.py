...
class FakeStore:
    def __init__(self):
        h = ContainerHealth(name="firefly", stack="firefly", state="running",
                             cpu_percent=2.2, mem_used=96_000_000, mem_limit=2_000_000_000,
                             mem_percent=4.8, restart_count=0, net_rx=1000, net_tx=2000)
        self._items = {"firefly_firefly": {
            "stack": "firefly", "name": "firefly", "health": h,
            "verdict": Verdict(True, "state=running cpu=2.2%", 96.0),
            "history": [(True, 1.0), (True, 2.0)], "updated": 2.0,
        }}
...
def test_index_has_controls_and_data_attributes():
    ...
    for marker in ['id="ag-q"', 'id="ag-group"', 'id="ag-sort"', 'id="ag-order"',
                   'data-mem="96000000"', 'data-cpu="2.2"', 'data-status="healthy"',
                   'Sort: Memory', 'autogatus.filters']:
